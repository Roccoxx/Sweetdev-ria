 ...
 * @type boolean
 */
SweetDevRia.EditableText.beforeValid  = function(){  /* override this */ return true;  };

/**
 * This method is called after validating the current editable text. 
 * To be overridden !!
 */
SweetDevRia.EditableText.afterValid = function(){  /* override this */ };
 ...
 * @type boolean
 */
SweetDevRia.EditableText.beforeClose  = function(){  /* override this */ return true;  };

/**
 * This method is called after closing the current editable text. 
 * To be overridden !!
 */
SweetDevRia.EditableText.afterClose = function(){  /* override this */ };
 ...
 * @type boolean
 */
SweetDevRia.EditableText.beforeActive  = function(elem){  /* override this */ return true;  };
 ...
 * @param {EditableElement} elem the element to activate 
 */
SweetDevRia.EditableText.afterActive = function(elem){  /* override this */ };
 ...
 * @private
 */ 
SweetDevRia.EditableText.prototype.initialize = function() {
...
SweetDevRia.EditableText.prototype.initElement  = function(elem, listenersId) {
	...
	if (elem!=null && elem.isInitialized!=true) {
...
SweetDevRia.EditableText.add  = function(id, editComp, value, srcId, listenersId, init) {
	...
	// Start Correction JIRA SWTRIA-914 by NIS
	// The element must be include portletId et be in component repository to ajax call
	elem.portletId = editableText.portletId; 
	SweetDevRia.set(id,SweetDevRia.COMPONENTS_REPOSITORY,elem);
	// End Correction JIRA SWTRIA-914 by NIS
	
//	SweetDevRia.EventHelper.addListener (window, "load", SweetDevRia.EditableText.prototype.init);
	
	SweetDevRia.toInitialize.add (editableText.id);
};
...
SweetDevRia.EditableText.setEditCompValue  = function(elem, value) {
	...
	if (type == SweetDevRia.Form.INPUT_TYPE) {
		comp.value = value.replace("&lt;","<").replace("&gt;",">");
		comp.focus ();
	}
...
SweetDevRia.EditableText.getEditCompValue  = function(elem) {
	...
	if (type == SweetDevRia.Form.INPUT_TYPE) {
	   ...
	   for (var i = 0; i < comp.length; i++) {
			if (comp [i].checked) {
				var label = SweetDevRia.Form.getLabel (comp [i].id);
				if (label) {
      				return label;
				}
				else {				
...
SweetDevRia.EditableText.setTextValue  = function(elem, value) {
	elem.innerHTML = value.replace("<","&lt;").replace(">","&gt;");
};
...
SweetDevRia.EditableText.active  = function(elem) {
	if(SweetDevRia.EditableText.beforeActive(elem)){
	
		if (elem) {
			var editableText = SweetDevRia.EditableText.getInstance();
			if (editableText.exist (elem.id) && editableText.activeElement != elem) {
...
SweetDevRia.EditableText.valid  = function() {
	if(SweetDevRia.EditableText.beforeValid()){
...
SweetDevRia.EditableText.close  = function() {
	if(SweetDevRia.EditableText.beforeClose()){
