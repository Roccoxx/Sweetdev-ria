...
SweetDevRia.EventHelper.LoadListeners=[];
SweetDevRia.EventHelper.PreLoadListeners=[];
SweetDevRia.EventHelper.PostLoadListeners=[];
 ...
 * @param {object}  a arbitraryObject that will be passed has an parameter to the handler
...
SweetDevRia.EventHelper.addListener = function(element, type, handler, oScope, bOverride) {
	if(type=="load"){
		SweetDevRia.EventHelper.LoadListeners.push([element, type, handler, oScope, bOverride]);
		return(true);
	} else
		{
 ...
 * @param {object}  a arbitraryObject that will be passed has an parameter to the handler
 * @param {boolean}  bOverride If true, the obj passed in becomes the execution scope of the listener
 * @return {int} The number of preLoad events actually registered.
 */
SweetDevRia.EventHelper.addPreLoadListener = function(element, handler, oScope, bOverride) {
	var type = "load";
	return SweetDevRia.EventHelper.PreLoadListeners.unshift([element, type, handler, oScope, bOverride]);
};
 ...
 * @param {object}  a arbitraryObject that will be passed has an parameter to the handler
 * @param {boolean}  bOverride If true, the obj passed in becomes the execution scope of the listener
 * @return {int} The number of postLoad events actually registered.
 */
SweetDevRia.EventHelper.addPostLoadListener = function(element, handler, oScope, bOverride) {
	var type = "load";
	return SweetDevRia.EventHelper.PostLoadListeners.push([element, type, handler, oScope, bOverride]);
};

/**
 * Run all the load events in the right order.
 */
SweetDevRia.EventHelper.runLoadListeners = function() {	
	
	for(var i=0; i<SweetDevRia.EventHelper.PreLoadListeners.length; i++) {
		var monListener=SweetDevRia.EventHelper.PreLoadListeners[i];
		var myHandler=monListener[2];
		if(monListener[4]) {
			myHandler.call(monListener[3],monListener[3]);
		} else
		{
			myHandler.call(monListener[0],monListener[3]);
		}
	}
	
	for(var i=0; i<SweetDevRia.EventHelper.LoadListeners.length; i++) {
		var monListener=SweetDevRia.EventHelper.LoadListeners[i];
		var myHandler=monListener[2];
		if(monListener[4]) {
			myHandler.call(monListener[3],monListener[3]);
		} else
	...
	for(var i=0; i<SweetDevRia.EventHelper.PostLoadListeners.length; i++) {
		var monListener=SweetDevRia.EventHelper.PostLoadListeners[i];
		var myHandler=monListener[2];
		if(monListener[4]) {
			myHandler.call(monListener[3],monListener[3]);
		} else
